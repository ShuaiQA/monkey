...
// evalProgram 对于当前的所有的程序进行求值
// 查找当前的所有的语句然后调用语句的求值操作,如果当前的程序出现了ReturnValue或者是Error可以直接退出
...
// 对语句块进行求值(一个语句块可以含有多个语句,始终注意表达式在本语言中是语句),返回对象操作
// 对每一个语句进行求值操作,如果结果不是空的,那么查看结果的类型
// 如果,当前的求值结果是RETURN_VALUE或ERROR则进行返回,最后的return好像不会到达(除非返回Null)
...
// 求值if表达式,首先会对于表达式中的condition(表达式)进行求值操作,如果当前的condition有错误直接返回错误
// 判断isTruthy是不是正确的是的话继续进行求值,不正确对另一个进行求值
...
// 对于中缀符号进行求值,先将object对象转换成Integer,然后获取值,进行对应的操作,返回object对象
// 比如是常规的运算操作在返回Integer对象,要是比较操作,返回Boolean对象
...
// 对负号操作符进行求值,注意当右面的类型不是整型的时候返回错误
...
// 判断当前的对象是不是返回true,如果值是空的和false返回false,其余返回true
